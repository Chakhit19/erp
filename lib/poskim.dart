...
// ══════════════════════════════════════════════════════════════
//  LOGIN SCREEN  (ตรงตาม Screenshot 15)
// ══════════════════════════════════════════════════════════════
...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Row(
        children: [
          // ── Left panel (purple gradient + features)
          Expanded(
            flex: 5,
            child: Container(
              ...
              child: Stack(
                children: [
                  Positioned(top: -60, left: -60, child: _circle(220, Colors.white.withOpacity(0.05))),
                  Positioned(bottom: -80, right: -80, child: _circle(300, Colors.white.withOpacity(0.05))),
                  Positioned(bottom: 120, left: 40, child: _circle(120, Colors.white.withOpacity(0.04))),
                  Center(
                    child: Column(
...
class ProductionLot {
  ...
  ProductionLot({
    required this.id, required this.productName, required this.category,
    required this.targetQty, required this.step, required this.createdAt,
  });
}
...
class InventoryMaterial {
  ...
  InventoryMaterial({required this.name, required this.quantity, required this.unit, required this.minLevel});

  bool get isLow => quantity <= minLevel;
}

// ══════════════════════════════════════════════════════════════
//  ROOT — สลับ Login กับ Main App
// ══════════════════════════════════════════════════════════════
...
class _MainAppState extends State<_MainApp> {
  ...
  static const _navIcons = [
    Icons.grid_view_rounded, Icons.inventory_2_outlined,
    Icons.history_rounded, Icons.analytics_outlined, Icons.settings_outlined,
  ];

  @override
  void dispose() { _productNameCtrl.dispose(); _qtyCtrl.dispose(); super.dispose(); }
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Padding(
          ...
          child: Row(children: [
            ...
            if (_sidebarOpen) ...[
              const SizedBox(width: 10),
              const Expanded(child: Text('KIMBUB',
                  style: TextStyle(fontSize: 15, fontWeight: FontWeight.w900, color: Colors.indigo, letterSpacing: 1.5))),
            ],
            IconButton(
        ...
        if (_sidebarOpen)
          Padding(padding: const EdgeInsets.only(left: 14, top: 2, bottom: 16),
              ...
              children: [
                ...
                if (_sidebarOpen) ...[
                  ...
                  Text(_navLabels[i], style: TextStyle(
                    fontSize: 13, color: active ? Colors.indigo : Colors.black54,
  ...
  Widget _buildTopBar() {
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        const Spacer(),
        if (lowStock > 0) _chip('วัตถุดิบใกล้หมด $lowStock รายการ', Colors.red.shade50, Colors.red),
        const SizedBox(width: 12),
        Stack(children: [
          IconButton(onPressed: _showNotifications, icon: const Icon(Icons.notifications_none, size: 22)),
          if (lowStock > 0) Positioned(right: 6, top: 6,
            child: Container(width: 8, height: 8, decoration: const BoxDecoration(color: Colors.red, shape: BoxShape.circle))),
        ]),
        GestureDetector(
          onTap: _showProfile,
          child: const CircleAvatar(backgroundColor: Colors.indigo, radius: 17,
              ...
              itemBuilder: (_, i) {
                ...
                return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Row(children: [
                    ...
                    Text(lot.step.label,
                        style: TextStyle(color: lot.step.color, fontWeight: FontWeight.bold, fontSize: 12)),
                    const SizedBox(width: 8),
                  ...
                  ClipRRect(
                    ...
                    child: LinearProgressIndicator(
                      value: lot.step.progress, color: lot.step.color,
                      backgroundColor: Colors.grey[100], minHeight: 7),
                  ),
                ]);
              }),
    );
  ...
  Widget _buildOrderForm() {
    return SizedBox(
      ...
      child: Column(children: [
        TextField(controller: _productNameCtrl, decoration: _inputDeco('ชื่อสินค้า', 'เช่น เสื้อยืด Basic White')),
          ...
          children: steps.map((s) {
            ...
            return Column(children: [
              AnimatedContainer(
                ...
                decoration: BoxDecoration(
                  color: isDone ? Colors.indigo : isCurrent ? Colors.indigo.withOpacity(0.15) : Colors.grey[100],
                  shape: BoxShape.circle,
                  border: isCurrent ? Border.all(color: Colors.indigo, width: 2) : null,
                ),
                child: Icon(s.icon,
                    color: isDone ? Colors.white : isCurrent ? Colors.indigo : Colors.grey[300], size: 20),
              ),
              const SizedBox(height: 5),
              Text(s.label, style: TextStyle(
                fontSize: 10, color: isDone || isCurrent ? Colors.black87 : Colors.grey,
  ...
  Widget _buildHistoryTable() {
    ...
    return SizedBox(
      ...
      child: done.isEmpty
          ...
          : Column(children: [
              Row(children: [_th('WO', 80), _th('สินค้า', 160), _th('จำนวน', 70), _th('สถานะ', 90)]),
  ...
  Widget _buildInventoryPage() {
    return SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const SizedBox(height: 16),
        ..._inventory.map((item) => _inventoryRow(item)),
      ]),
    );
  }
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        Text('${item.quantity} ${item.unit}',
            style: TextStyle(fontWeight: FontWeight.bold, color: item.isLow ? Colors.red : Colors.black87)),
        if (item.isLow) ...[const SizedBox(width: 8), _chip('ใกล้หมด', Colors.red.shade50, Colors.red)],
        const SizedBox(width: 12),
        IconButton(
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        SizedBox(width: 120, child: Row(children: [
          if (lot.step != ProductionStep.done)
            IconButton(icon: const Icon(Icons.arrow_forward_ios, size: 15, color: Colors.indigo),
              onPressed: () => setState(() => lot.step = ProductionStep.values[lot.step.index + 1])),
          IconButton(icon: const Icon(Icons.visibility_outlined, size: 16, color: Colors.teal),
  ...
  Widget _buildQCPage() {
    ...
    return SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const SizedBox(height: 24),
        _card('สรุป QC แต่ละ Lot', Column(children: [
          if (done.isEmpty)
    ...
    return SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        _card('ข้อมูลโรงงาน', Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          const SizedBox(height: 12),
          ElevatedButton(onPressed: () => _showSnack('บันทึกข้อมูลเรียบร้อย'),
            style: ElevatedButton.styleFrom(backgroundColor: Colors.indigo,
                shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8))),
  ...
  void _submitOrder() {
    ...
    if (name.isEmpty || qty <= 0) { _showSnack('กรุณากรอกชื่อสินค้าและจำนวนให้ถูกต้อง'); return; }
    setState(() {
      _lots.insert(0, ProductionLot(
        id: 'WO-${_nextWO.toString().padLeft(3, '0')}', productName: name,
        category: _selectedCategory, targetQty: qty, step: ProductionStep.cutting,
        createdAt: DateTime.now()));
      _nextWO++;
      _productNameCtrl.clear(); _qtyCtrl.clear();
    });
    ...
    showDialog(context: context, builder: (_) => AlertDialog(
      ...
      actions: [
        ...
        ElevatedButton(onPressed: () {
          setState(() => _inventory.add(InventoryMaterial(
            name: nameCtrl.text, quantity: double.tryParse(qtyCtrl.text) ?? 0,
            unit: unitCtrl.text, minLevel: double.tryParse(minCtrl.text) ?? 0)));
          Navigator.pop(context); _showSnack('เพิ่มวัตถุดิบสำเร็จ');
    ...
    showDialog(context: context, builder: (_) => AlertDialog(
      ...
      actions: [
        ...
        ElevatedButton(onPressed: () {
          ...
          Navigator.pop(context); _showSnack('อัพเดตสำเร็จ');
        }, child: const Text('บันทึก')),
      ],
    ));
  }
    ...
    showDialog(context: context, builder: (_) => AlertDialog(
      ...
      actions: [
        ...
        ElevatedButton(onPressed: () {
          ...
          Navigator.pop(context); _showSnack('ลบเรียบร้อยแล้ว');
  ...
  void _showNotifications() {
    ...
    showDialog(context: context, builder: (_) => AlertDialog(
      ...
      content: low.isEmpty ? const Text('ไม่มีการแจ้งเตือน')
  ...
  void _showProfile() {
    showDialog(context: context, builder: (_) => AlertDialog(
      ...
      content: const Column(mainAxisSize: MainAxisSize.min, children: [
        CircleAvatar(backgroundColor: Colors.indigo, radius: 30,
          child: Icon(Icons.person, color: Colors.white, size: 30)),
        SizedBox(height: 12),
        Text('Admin', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
        Text('admin@kimbubfactory.com', style: TextStyle(color: Colors.grey, fontSize: 12)),
      ]),
      actions: [
        TextButton(onPressed: () => Navigator.pop(context), child: const Text('ปิด')),
        ElevatedButton(onPressed: () {
          Navigator.pop(context); widget.onLogout();
        }, style: ElevatedButton.styleFrom(backgroundColor: Colors.red),
